...
public class MclcConf extends PApplet {
...
public void activateButton(int idx){
  idx = max(0, idx);
  idx = idx%maxColors;
  activeButton = idx;
...
public void createColorButtons(List<Integer> colors, int startX, int startY, int buttonX, int buttonY){
...
public void createLedButtons(List<Integer> leds, int startX, int startY, int buttonX, int buttonY, int offsetX, int offsetY){
...
public void activateConfig(int config){
...
public void setColorToButton(Button bb, int c){
  bb.setColorActive(c);
...
public void InitSerial(int portValue) {
  if (initCom){
    g_serial.clear();
    g_serial.stop();
  }
...
public void setColors(){
  ...
  for (int i=0; i<maxColors; i++){
    ...
    int g = (int)green(col);
    int b = (int)blue(col);
    payload.add(PApplet.parseChar(r));
    payload.add(PApplet.parseChar(g));
    payload.add(PApplet.parseChar(b));
  }
...
public void getConfigs(){
  for (int i=0; i<maxConfigs; i++){
    List<Character> payload = new ArrayList<Character>();
    payload.add(PApplet.parseChar(i));
...
public void setConfigs(){
  List<Character> payload = new ArrayList<Character>();
  int value = 0;
  payload.add(PApplet.parseChar(activeConfig));
  for (int i=0; i<numLeds; i++){
    if (i%2==0){
      ...
      payload.add(PApplet.parseChar(value));
      value = 0;
    }
  }
  payload.add(PApplet.parseChar(value));
...
private void readSerialMessages(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "MclcConf" };
    if (passedArgs != null) {
